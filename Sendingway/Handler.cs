...
namespace Sendingway
{
    internal class Handler : IDisposable
    {
        private const string ServerUrl = "http://127.0.0.1:5000/";  // update this URL when we have the server
        ...
        {
            if (pfListingsQueue.Count == 0 || cooldown.ElapsedMilliseconds < 10000 || lockout.ElapsedMilliseconds < 1000)
            {
                return;
            }
